...
# --- 超参数设置 (MacBook M4 原型验证版) ---

ITERATIONS = 5  # 完整版建议: 200 - 500
TRAVERSALS_PER_ITER = 5  # 完整版建议: 100,000 - 300,000
BATCH_SIZE = 512  # 完整版建议: 10240
UPDATE_STEPS = 100  # 完整版建议: 4000
BUFFER_CAPACITY = 200000  # 完整版建议: 40,000,000 (需要约10GB+内存)
...
def traverse(env, history, traverser, iteration, nets, buffers):
    ...
    # ==========================================
    if turn != traverser:
        # 注意：这里从 3 改为了 5，对应 5 种可能的动作概率分布
...
if __name__ == "__main__":
    print("🤖 启动 SD-CFR 训练 (多档位限注德州扑克 原型版)...")

    # 初始化最新的德州扑克环境
    env = TexasEnv()

    # 实例化最新扩容版的网络 (Input:111, Output:5)
